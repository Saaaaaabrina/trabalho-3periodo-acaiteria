...
void salvarPedidoEmArquivo(Pedido p, const char* metodo) {
    FILE* arquivo = fopen("pedidos_dia.txt", "a");
    if (arquivo == NULL) {
        printf("Erro ao abrir o arquivo para salvar o pedido.\n");
        return;
    }

    fprintf(arquivo, "Pedido: %s\n", p.descricao);
    fprintf(arquivo, "Quantidade: %d\n", p.quantidade);
    fprintf(arquivo, "Total: R$ %.2f\n", p.preco);
    fprintf(arquivo, "Pagamento: %s\n", metodo);
    fprintf(arquivo, "-------------------------------\n");
...
int main() {
    ...
    while (rodando) {
        ...
        if (escolha == 1) {
            ...
            const char* metodoStr = (metodo == 1) ? "Dinheiro" : "Cartão";
            ...
            salvarPedidoEmArquivo(p, metodoStr); // <-- Aqui salvamos no arquivo
